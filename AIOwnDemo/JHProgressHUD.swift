...
class JHProgressHUD: UIView {
	...
	var footerColor: UIColor = UIColor.whiteColor()
...
//		loaderColor = UIColor.whiteColor()
	...
	func showInWindow(withHeader title: String?, andFooter footer: String?) {
		self.showInView(app.window!, withHeader: title, andFooter: footer)
	}

	func showInWindow() {
		self.showInWindow(app.window!)
	}
	...
	private func isCleanedStringEmpty(string: String) -> Bool {
		let cleanString = string.stringByTrimmingCharactersInSet(NSCharacterSet.whitespaceCharacterSet())
